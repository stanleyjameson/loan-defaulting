...
def calculate_credit_score(annual_income, work_exp, years_in_current_employment, marital_status, house_ownership, vehicle_ownership):
    # Initializing the credit score with a base value
    credit_score = 500
    # The higher the income, the greater the increment
    ...
    # Experience in work adds to the credit score
    credit_score += work_exp * 5
    
    # Stability in employment positively impacts the score
    if years_in_current_employment < 5:
        credit_score += 20
        
    # Marital status can have a slight effect on the credit score
    if marital_status == "Married":
        credit_score += 10
        
    # Owning a house significantly increases the score
    if house_ownership == "Owned":
        credit_score += 50
    elif house_ownership == "Rented":
        credit_score += 20
        
    # Having a vehicle shows financial responsibility, thus increasing the score
            ...
            with content:
                ...
                with col1:
                    with st.form("Preidct"):
                        c1, c2 = st.columns(2)
                        with c1:
                            annual_income = st.number_input('**Annual Income**', min_value=5000,
                                                            max_value=1000000000, value=100000)

                            applicant_age = st.number_input(
                                '**Applicant Age**', min_value=16, max_value=85, value=33)
